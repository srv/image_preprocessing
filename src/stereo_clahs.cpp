...
StereoClahs::StereoClahs(ros::NodeHandle nh, ros::NodeHandle nhp)
  ...
  left_mono_pub_   = it.advertiseCamera(
    ros::names::clean(stereo_ns + "/clahs/" + slave_ns + "/image_rect"),  1);
  right_mono_pub_  = it.advertiseCamera(
    ros::names::clean(stereo_ns + "/clahs/" + master_ns + "/image_rect"), 1);
  left_color_pub_   = it.advertiseCamera(
    ros::names::clean(stereo_ns + "/clahs/" + slave_ns + "/image_rect_color"),  1);
  right_color_pub_  = it.advertiseCamera(
    ros::names::clean(stereo_ns + "/clahs/" + master_ns + "/image_rect_color"), 1);
